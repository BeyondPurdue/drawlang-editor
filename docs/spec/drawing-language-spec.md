...
**Owner:** BM Global
**Document status:** LOCKED v0.6 — frozen 2026-08-09. Any change requires a new version (v0.7).

> **Not affiliated with Siemens.** This specification describes DrawLang, an independent language designed by BM Global for the Beyond Purdue editor. It cites historical Siemens ES680 pictogram behaviour in rationale sections purely to justify grammar decisions on the basis of observed real-world drawing programs. No Siemens source code, manuals, or database exports are reproduced here. Siemens®, TXP®, SPPA-T3000®, and ES680® are trademarks of their respective owners.


**Change from v0.5 → v0.6:** the palette model is now defined explicitly by the specification, not left to the backend. Palette index 0 is reserved as `paper` — the background / non-color slot — and a fill that resolves to palette 0 MUST render as invisible (no fill emitted). A bare `,f` modifier with no `,c<n>` MUST default its fill index to 0 (paper); therefore `rt,W,H,f` and `ci,r,f` draw an outline only, matching the historical Siemens ES680 HMI behaviour where palette entry 0 was the workstation background colour. Stroke and text default to palette index 1 (`ink`, black in the reference palette) when no `,c<n>` is present, so a bare `dl,dx,dy` still draws a visible line — the paper/ink asymmetry is deliberate. `,f,c0` is an explicit paper fill and is equivalent to bare `,f`; `,f,c<n>` with *n* ≥ 1 fills with palette[*n*]. This change is grammar-preserving and interpreter-preserving — only backend colour resolution changes — and every v0.5 program remains a valid v0.6 program. Rationale: real ES680 pic_ex symbols use bare `,f` (e.g. `pic_ex -1` contains `rt,1224,854,f` and `rt,1222,852,f`) to draw outlined title-block rectangles; the v0.5 backend rendered these as large solid black blocks, which is not what the source HMI does. See §5.3, §6.4, §6.5, §8.1, §14.
...
- **Ink** — palette index **1**. The default drawing color used for strokes and text when no `,c<n>` modifier is present. In the reference palette this is black (`#000000`); real ES680 projects may override this via the palette table.
...
The paper/ink asymmetry is deliberate. Without it, a bare `dl,50,50` would resolve its stroke to palette 0 and produce nothing, which contradicts every observed ES680 program. With it, a bare `dl` draws in ink and a bare `rt,W,H,f` fills with paper (invisible) — which is exactly what the source HMI does.
...
```
Reserves a 24×25 bounding box then walks an invisible L-path to extend the extent; nothing renders but the drawing's bounding box now includes those endpoints. Pattern taken from real ES680 pic_ex 24904.
...
ma,10,10; rt,80,40,f;
```
Draws the same rectangle, outline only — the bare `,f` resolves to `paper` (invisible) in v0.6. Matches real ES680 title-block rectangles.
...
- **Paper.** The background / non-color slot — palette index 0. Reserved: a fill that resolves to paper is not painted (renders as invisible). Makes bare `,f` — fill with no color — mean "outline only", matching the source ES680 HMI where palette 0 was the workstation background.
