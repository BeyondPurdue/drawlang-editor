...
**Project:** Beyond Purdue Editor — Universal Vector Drawing Language
**Owners:** Beyond Purdue contributors
**License:** Apache 2.0
**Document status:** LOCKED v0.1 — frozen 2026-08-09. Any change requires a new version (v0.2).
...
- **Float.** A signed decimal number with an explicit decimal point. Grammar: `-?[0-9]+\.[0-9]*` or `-?\.[0-9]+`. Example: `0.`, `90.0`, `-.5`, `3.14159`.
...
Comments are not part of the language. Programs contain only opcodes and arguments. Documentation of program intent lives outside the program, in the surrounding database schema (for example, a description column adjacent to the cmd column).
...
**Semantics:** Draws a straight line from the current pen position to `(current_x + dx, current_y + dy)`. Line width is defined by the backend's default stroke width (typically 1 unit); backends MAY offer configuration, but the language does not carry stroke width.
...
| # | Name | Type | Meaning |
|---|---|---|---|
| 1 | angle | float | Rotation in degrees, counterclockwise. `0.` = horizontal, reading left-to-right. `90.` = rotated 90° CCW (reading bottom-to-top). |
...
|---|---|---|---|
| 1 | r | integer | Radius. Must be positive. |
| 2 | start_angle | float | Angle at which the arc begins, in degrees. `0.` = pointing right along +X. |
| 3 | sweep_angle | float | Arc extent, in degrees. Positive = counterclockwise, negative = clockwise. A sweep of `360.` (or `-360.`) yields a full circle, semantically identical to `ci,r`. |
...
| `,i` | Invisible / atmend boundary | `rt` | Marks the shape as contributing to bounding-box computation but not rendered. Used to define expandable-block boundaries. |
...
|---|---|---|
| `symbol` | `cmd` | Symbol definition (a reusable graphical block). |
| `extsym` | `cmd` | Extended symbol — used when a symbol renders differently in the palette vs. on the drawing. Also used for **frame elements**: paper border, grid lines, corner marks, title-block outline. |
| `titleblock` | (parameters) | Title-block field data (labels and coordinates for the fields inside the frame's title block). |
| `instance` | (references + placement) | An instance of a symbol placed on a drawing at a specific position, angle, and mirror state. |
| `connector` | (endpoints) | A connection (wire) between two symbol instances. Rendered as a sequence of `dl` operations by the interpreter's wiring subsystem. |
| `label` | `cmd` | Free-standing text label. |
...
A single cmd string is up to 256 characters. When a drawing element is complex enough to exceed this (typical for symbols with many strokes), multiple rows are used, distinguished by a sequence number (`seq`). The interpreter concatenates rows in order of `seq` before executing.
...
Current version: **0.1 (locked, 2026-08-09)**. Approved by the project owner as the frozen reference. All interpreters and programs are built against this version.
...
- **Block.** An expandable region whose bounding rectangle is defined by an invisible (`,i`) `rt` and whose contents can grow within that bound.
- **cmd string.** The text of a program, as stored in a database column.
- **Compliant / conforming.** Following this specification exactly.
- **Connector.** A connection (wire) between two placed symbols; rendered as a sequence of `dl` operations.
...
- **Frame.** The border of a drawing sheet, including the drawing area and margins.
- **Grid.** The reference labels (A/B/C/… rows and 1/2/3/… columns) along the border of a frame.
- **Interpreter.** A software component that reads a cmd string and produces drawing output.
- **Mask.** A named group of parameters that decorates a title-block field or symbol instance.
...
- **Symbol.** A reusable graphical block, defined by one or more `symbol` rows and placed by `instance` rows.
- **Title block.** The metadata box on a drawing sheet (project, drawing number, revision, etc.).
